...
| AI校正 | `/notes/<id>/ai/review`, `/notes/<id>/ai/result` |
| AI報告書 | `/projects/<id>/ai/report/review`, `/projects/<id>/ai/report/result` |
...
アップロードは同一保存先内の一時ファイルへ書き込みと `fsync` を行い、DBトランザクション中に行を登録してから原子的に最終名へ移動し、DBをコミットする。移動失敗はDBをロールバックし、DBコミット失敗は最終ファイルを削除する。
...
確認本文やAI結果はSQLite、ログ、セッションCookieへ保存しない。確認画面から実行画面へは本文をPOSTで再送し、セッションにはランダムな同意nonce、本文のSHA-256、対象ID、表示時の更新日時、有効期限だけを保持する。実行時に本文ハッシュ、nonce、有効期限、更新日時を再検証してから1回だけ送信する。
